...
def train_round_fedpart(args, global_model, local_clients, rnd, **kwargs):
    print(f'\n---- Global Communication Round : {rnd + 1} ----')
    num_users = args.num_users
    m = max(int(args.frac * num_users), 1)
    if (rnd >= args.epochs):
    ...
    # global_weight = global_model.state_dict()       # load all parameters
    # global_weight = global_model.fc2.state_dict()   # load classifer parameters
    # load feature extractor parameters
    global_weight = global_model.state_dict()
    global_weight.pop('fc2.weight')
    global_weight.pop('fc2.bias')

    for idx in idx_users:
        local_client = local_clients[idx]
        agg_weight.append(local_client.agg_weight)
        local_epoch = args.local_epoch
        local_client.update_local_model(global_weight=global_weight)   # update local classifier parameters
        w, loss1, loss2, acc1, acc2 = local_client.local_training(local_epoch=local_epoch, round=rnd)
        local_weights.append(copy.deepcopy(w))
        local_losses1.append(copy.deepcopy(loss1))
        local_losses2.append(copy.deepcopy(loss2))
    ...
    # global_model.fc2.load_state_dict(global_weight)    # update global classifier parameters
